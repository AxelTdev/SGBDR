...
public class DiskManager {
	...
	//private static File f =null; //on ne doit pas déclarer le Fichier ici sinon on ne pourra plus accéder aux autres fichiers créés
	...
	public static PageId AddPage(int fileIdx) throws IOException {
		...
		try {
			...
			//on se positionne au bon index (à la bonne page) pour ajouter une nouvelle page
	...
	public static void WritePage(PageId p, byte[] buff) {
		RandomAccessFile r = null;
		try {
			File f = new File("src/DB/Data_" + p.getFileIdx() + ".rf");
		...
		try {
			...
			// on se positionne à la bonne page pour commencer à écrire
			r.seek(Constants.pageSize*(p.getPageIdx()-1));
			r.write(buff);
			r.close();
		} catch (IOException e) {
	...
	public static void main(String[] args) throws IOException {
		...
		PageId pageId2 = DiskManager.AddPage(1);
		byte[] buff2 = new byte[Constants.pageSize];
		String str2 = "fuck";
